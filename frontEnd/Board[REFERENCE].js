function Board {
    ...
    let uberRow = {0:0}, atkRow = {0:0}, cDefRow = {0:0}, coreBlock = {0:0}, defRow = {0:0};
    currentBuild = ComboBuild.ComboBuild;
    isThereBuild = false;
    Board(){ board_Grid = new Positions[4][3]; } //4 rows 3 col

    let getUberRow = uberRow; 
    let getAtkRow = atkRow; 
    let getCDefRow = cDefRow;
    let getCoreBlock = coreBlock;
    let getDefRow = defRow;

    function calculate(enemy){
        for(var f = 0; f < 2; f++){
            for(var r = 0; r < 3; r++){
